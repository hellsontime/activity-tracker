    ...
    <div class="p-8 flex items-start bg-light-grey rounded-md shadow-lg">
      ...
      <form @submit.prevent="createWorkout" class="flex flex-col gap-y-5 w-full">
        ...
        <div class="flex flex-col">
          <label for="workout-name" class="mb-1 text-sm text-at-light-green">Workouts</label>
        ...
        <div class="flex flex-col ">
          ...
          <select id="workout-type" class="p-2 text-grey-500 bg-white focus:outline-none" required v-model="workoutType" @change="workoutChange">
            <option value="select-workout">Select Workout</option>
            <option value="strength">Stength Training</option>
        ...
        <div v-if="workoutType === 'strength'" class="flex flex-col gap-y-4">
          ...
          <button @click="addExercise" class="mt-6 py-2 px-6 rounded-sm self-start text-sm text-white bg-at-light-green duration-200 border-solid border-2 border-transparent hover:border-at-light-green hover:bg-white hover:text-at-light-green">Add Exercise</button>
        </div>
        ...
        <div v-if="workoutType === 'cardio'" class="flex flex-col gap-y-4">
          <div class="flex flex-col gap-x-6 gap-y-2 relative md:flex-row" v-for="(item, index) in exercises" :key="index">
            <div class="flex flex-col md:w-1/3">
              ...
              <select id="cardio-type" class="p-2 w-full text-grey-500 bg-white focus:outline-none" v-model="item.cardioType">
